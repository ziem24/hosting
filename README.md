...
- Downloading a server from a cloud storage
...
- Each server directory must contain an executable 'run.sh' or 'start.sh' script (use chmod +x to grant executable permissions). This script will be later used to run the server (if, for some reason, both scripts are present, then 'run.sh' will take priority).
...
- Configuration file (hosting.conf): this is where the config options are stored
